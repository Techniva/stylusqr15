...
import { Save, Database, RefreshCw, Lock, LogIn, QrCode, Calendar, Edit3, Download, ExternalLink, Play, Globe, Eye, Trash2, BarChart2 } from "lucide-react";
import { handleAuthError } from '@/lib/authUtils';
import { formatQRDataToURL } from '../../../lib/qrDataUtils';
import QRAnalyticsReport from './QRAnalyticsReport';
...
const MyQRCodes: React.FC<MyQRCodesProps> = ({ url, onUrlChange, user = null, onStatsChange }) => {
  ...
  const reportRefs = useRef<{ [qrId: number]: HTMLDivElement | null }>({});
  ...
  const handleDownloadReport = async (qr: QRCode) => {
    const stats = qrStats[qr.id] || { last7Days: {}, last6Months: {}, devices: {} };
    // Dummy performance data for now (replace with real if available)
    const performance = {
      top: [
        { place: 'Delhi, India', count: 75 },
        { place: 'New York, USA', count: 120 },
        { place: 'London, UK', count: 98 },
      ],
      bottom: [
        { place: 'Oslo, Norway', count: 2 },
        { place: 'Lisbon, Portugal', count: 3 },
        { place: 'Hanoi, Vietnam', count: 4 },
      ],
    };
    ...
    const report = (
      <QRAnalyticsReport
        logoUrl={'/file.svg'}
        qrImage={qr.qrCodeImagePath || '/placeholder.png'}
        businessType={'Business'}
        qrType={'Dynamic QR'}
        createdAt={new Date(qr.createdAt).toLocaleDateString()}
        updatedAt={new Date(qr.updatedAt).toLocaleDateString()}
        activeLink={formatQRDataToURL(qr.qrData)}
        lastLink={qr.lastLink}
        dynamicLink={qr.serverLink}
        scans={qrScanCounts[qr.id] ?? 0}
        updates={qr.updateCount}
        status={qr.qrStatus}
        scanActivity={stats.last7Days}
        yearlyActivity={stats.last6Months}
        performance={performance}
        deviceAnalytics={stats.devices || {}}
      />
    );
    // Render to container
    ...
    pdf.save(`qr-analytics-report-${qr.id}.pdf`);
    root.unmount();
  ...
  // ... (rest of the component logic from ChangeUrlTab, unchanged except for the component name)
                ...
                <div className="flex flex-row items-center min-w-[180px]">
                  ...
                  <div className="flex flex-col justify-center pl-4">
                    <div className="flex items-center text-xs text-gray-500 mb-0.5">
                      <span>Type: {qr.qrData.type}</span>
                    </div>
                    <div className="text-xs text-orange-600 font-semibold mb-0.5">Dynamic QR</div>
                    ...
                    <div className="absolute top-0 left-4 -translate-y-1/2 bg-white px-2 text-sm text-gray-700">
                      ...
                      <div className="flex items-end gap-2 h-32">
                        {qrStatsLoading[qr.id] ? (
                          ...
                          Object.entries(qrStats[qr.id].last7Days).map(([label, value], idx, arr) => {
                    ...
                    {/* Yearly (Last 6 Months) */}
                      ...
                      <div className="flex flex-col items-center">
                        {qrStatsLoading[qr.id] ? (
                          ...
                          <>
                            <svg width="220" height="90" viewBox="0 0 240 90" className="mb-2">
                              <polyline
                                fill="none"
                                stroke="#22c55e"
                                strokeWidth="3"
                              ...
                              />
                              {Object.values(qrStats[qr.id].last6Months).map((v, i, arr) => {
                                const max = Math.max(...Object.values(qrStats[qr.id].last6Months), 1);
                                const min = Math.min(...Object.values(qrStats[qr.id].last6Months));
                                const y = 80 - ((v - min) / (max - min || 1)) * 60;
                                const x = 20 + i * 40;
                    ...
                    {/* Device Analytics Pie Chart (always show) */}
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <div className="flex flex-row items-start mt-2 mb-2">
                          {Object.entries(qrStats[qr.id]?.devices || {}).map(([os, count], i) => (
              ...
              <div className="p-6">
                <div className="mb-4">
                  ...
                  <div className="w-full px-3 py-2 rounded bg-gray-50 text-green-700 font-semibold text-sm break-all border border-gray-200 cursor-not-allowed select-all">
                    {qrCodes.find(q => q.id === editingQRId) ? formatQRDataToURL(qrCodes.find(q => q.id === editingQRId)!.qrData) : ''}
                  </div>
                </div>
                <div className="mb-4">
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    onClick={() => handleSaveEdit(qrCodes.find(q => q.id === editingQRId)!)}
